...
def file_to_sfixed(cllct_filename, data_IntWidth):
    ...
    data_str = np.loadtxt(fname=cllct_filename, dtype=dt_string, delimiter="\n,",ndmin=1)
...
def testbench_layer(layer_folder, neuronweight_IntWidth, neuronbias_IntWidth, neuroninput_IntWidth,act_fun):
    ...
    layer_folder_path = os.getcwd()+"\..\\\\files\\\\"+layer_folder
    ...
    if act_fun == "ReLU":
        ...
        for c in range(number_neurons):
            ...
            if w_sums[c] >= 0:
                LUT_index=-1+2**(sigmoid_inputSize-1)+genSigmoid.float_to_fp_10(w_sums[c], sigmoid_inputSize, sigmoid_inputSize-sigmoid_inputIntSize)+1#+2 is to get the cardinal position inside the LUT. -1 is because Python indexes starts from 0.
            else:
                LUT_index=-2**(sigmoid_inputSize-1)+genSigmoid.float_to_fp_10(w_sums[c], sigmoid_inputSize, sigmoid_inputSize-sigmoid_inputIntSize)
...
def create_layer_tb(layer_folder,num_inputs, num_outputs, input_dataWidth, input_dataIntWidth, weight_dataWidth, weight_dataIntWidth, sigmoid_inputSize, sigmoid_inputIntSize,act_fun):
    ...
    tb_path = "..\\\\files\\\\"+layer_folder+"\\\\TB_VHDL_output"
