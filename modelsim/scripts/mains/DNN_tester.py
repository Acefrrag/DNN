...
net = network2.load(wb_path)
validation_data = list(validation_data)
training_data = list(training_data)
test_data = list(test_data)

i = test_data_sample
out = net.feedforward(test_data[i][0])#The target for this input is 8.
print("The output of  the DNN for test_data_sample="+str(test_data_sample)+" is:")
print(out)
print("The resulting number for the input with target "+ str(test_data[i][1]) +" is " + str(np.argmax(out)) + "\n")
