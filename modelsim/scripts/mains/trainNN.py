...
selection = int(misc.read_integer())
if selection == 0:
    act_fun_type = "ReLU"
else:
    act_fun_type = "Sig"
...
net = network2.Network([num_inputs[0]]+num_outputs, act_fun_type=act_fun_type)#change the number of layers or number of neurons in each layer here
...
try:
    ...
    f.write("constant act_fun_type: string  := \""+act_fun_type+"\";\n")
...
try:
    ...
    f.write("addr_out: in std_logic_vector(0 to natural(ceil(log2(real(layer_outputs(3)))))-1)); --To scan through the valdation data set\n")
    f.write("end DNN;\n")
    #Begin Architecture
    ...
    for i in range(num_hidden_layers):
        ...
        f.write("act_fun_type => act_fun_type,\n")
