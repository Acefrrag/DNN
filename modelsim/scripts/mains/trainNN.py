...
Outputs:
        A folder with relative path "../files/weights_n_biases/training_"+<date>+"/VHDL_output" containing the following files:
            -weights:               Folder containing weights for every neuron for every layer
            -biases:                Folder containing the biases for every neuron for every layer
            -DNN_package.vhd:       VHDL package containing data sizes DNN paramters sizes per layer etc..
            -DNN.vhd:               VHDL package which contain,s the reconfigured DNN
            -training_report.txt:   Training report of the generated DNN
            -dataFormatFile.txt:    File containg Size of bias and weights
            -WeightsandBiases.txt:  File containing the weights and biases of the DNN.
            -biasValues.h:          C Header with an array containing all the bias. For C application development
            -weightValue.h:         C Header
        A folder with relative path "../files/sigmoid" containing:
            -SigConten.mif: Containing the sigmoid ROM content.
        
Usage:
    Once output is ready:
        -the folders weights and biases must be put in the folder tb_files/tb<ID>
        -the VHDL files DNN.vhd and DNN_package.vhd must be put in src fileù
        -sigmoid folder inside ../../tb_files/DNN/tb<ID>
    Remember: The tb_files must contain the script generated by the script
...
num_outputs_DNN = 10
#HYPERPARAMETERS
#For more information look at network2 documentation
epochs = 1
batch_size = 100
eta = 0.4
lmbda = 5.0
...
#Input format: training data, number of iterations, batch size,eta(Learning Rate), lambda
...
#SIZES
#Neuron weight and output sizes
